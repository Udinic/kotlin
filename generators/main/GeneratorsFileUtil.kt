...
object GeneratorsFileUtil {
    ...
    fun writeFileIfContentChanged(file: File, newText: String, logNotChanged: Boolean = true) {
        ...
        if (!parentFile.exists()) {
            if (isTeamCityBuild) return assertTeamCityMode("Create dir: ${parentFile.path}")
        ...
        if (isTeamCityBuild) return assertTeamCityMode("Write file: ${file.toPath()}")
    ...
    fun assertTeamCityMode(message: String) {
        System.err.println("You should commit all newly generated files before pushing them to TeamCity\n$message")
    }
